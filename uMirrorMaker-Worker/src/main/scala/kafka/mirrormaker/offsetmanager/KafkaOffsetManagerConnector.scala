...
private[offsetmanager] class KafkaOffsetManagerConnector(brokers : Seq[BrokerEndPoint], socketTimeoutMs: Int, backoffMs: Int, maxRetries: Int) {
  ...
  /**
    ...
    *
    * @param broker - Broker that will be used for the channel creation
    * @return - Blocking channel for the broker's connection
    */
  private def getDefaultChannel(broker: BrokerEndPoint): BlockingChannel =
    new BlockingChannel(broker.host, broker.port, BlockingChannel.UseDefaultBufferSize,
  ...
  /**
    ...
    * @param brokers - Brokers that we will be asked for consumer group coordinator
    * @return Option(BrokerEndPoint) if coordinator found otherwise None
    */
  private def getGroupCoordinator(consumerGroup: String, brokers: Seq[BrokerEndPoint] = brokers): Option[BrokerEndPoint] = {

    log.info(s"Looking for coordinator of consumerGroup $consumerGroup in brokers: ${brokers.map(_.connectionString())}")

    brokers.foreach(broker => {

      log.info(s"Asking broker: ${broker.connectionString()} for coordinator. " +
        s"consumerGroup: $consumerGroup, correlationID: ${correlationId.get}.")

      val channel = getDefaultChannel(broker)
      ...
      if(byteBufferResponse.isDefined) {
        ...
        log.warn(s"Failed to get response from broker: ${broker.connectionString()} " +
  ...
  /**
    ...
    * @param brokers - Broker to commit
  ...
  def commit(consumerGroup : String,
             ...
             brokers : Seq[BrokerEndPoint] = brokers) : Unit =
